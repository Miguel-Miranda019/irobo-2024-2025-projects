...
class ErrorCalculator:
    ...
    def plot_results(self):
        """Plot the position errors and confidence intervals over time."""
        plt.figure()

        # Plot X error with confidence interval
        plt.subplot(2, 1, 1)
        plt.plot(self.times, self.errors_x, label='Error X')
        # Confidence interval: [error - uncertainty, error + uncertainty]
        plt.fill_between(self.times,
                         np.array(self.errors_x) - 2 * np.array(self.uncertainties_x),
                         np.array(self.errors_x) + 2 * np.array(self.uncertainties_x),
        ...
        # Plot Y error with confidence interval
        plt.subplot(2, 1, 2)
        plt.plot(self.times, self.errors_y, label='Error Y')
        plt.fill_between(self.times,
                         np.array(self.errors_y) - 2 * np.array(self.uncertainties_y),
                         np.array(self.errors_y) + 2 * np.array(self.uncertainties_y),
